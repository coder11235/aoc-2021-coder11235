// implementing a stack trace of sorts
...
type StackTrace = Vec<(StateParts, i32)>;

// states hashmap used for memoization
struct States {
    states: HashMap<StateParts, Option<(i32, StackTrace)>>
}
impl States {
    fn get_state(&self, state: &StateParts) -> Option<(i32, StackTrace)> {
    ...
    fn update_state(&mut self, state: &StateParts, min_cost: i32, stack_trace: StackTrace) {
        self.states.insert(state.clone(), Some((min_cost, stack_trace)));
    }
    pub fn new() -> Self {
...
pub fn part2(input: &String) -> i32 {
    ...
    println!("{:?}", init_state);
    println!("{:?}", move_amp(&mut States::new(), init_state, 0));
    5
}
...
fn move_amp(states_mem: &mut States, state: StateParts, cost_so_far: i32) -> (i32, StackTrace) {
    // impl memoization
    if states_mem.check_state(&state) {
        match states_mem.get_state(&state) {
            None => return (f64::INFINITY as i32, Vec::new()),
            Some(val) => return val
        }
    }
    else {
    ...
    let mut least_stack: Vec<(i32, StackTrace)> = Vec::new();
    ...
    for (h_num, amp) in hallway_ref.iter().enumerate() {
        ...
        if let Some(amp_home_row) = home_occupance(*amp, rows_ref) {
            ...
            least_stack.push(move_amp(states_mem, cloned_state, cost_so_far + cost as i32))
        }
    }
    ...
    for col_num in 0..4 {
        ...
        least_stack.append(&mut move_into_hall(cost_so_far, opp_hallway_pos+1..11, amp_row_num as usize, col_num, &state, states_mem));
        least_stack.append(&mut move_into_hall(cost_so_far, (0..opp_hallway_pos).rev(), amp_row_num as usize, col_num, &state, states_mem));
    }

    //let minimum_cost = *least_costs.iter().min().unwrap_or(&(f64::INFINITY as i32));
    let mut min_cost = f64::INFINITY as i32;
    let mut min_stack: Option<StackTrace> = None;
    for (cost, stack) in least_stack {
        if cost < min_cost {
            min_cost = cost;
            min_stack = Some(stack);
        }
    }
    if min_stack == None {
        return (cost_so_far, Vec::new());
    }
    let min_stack = min_stack.unwrap();
    min_stack.push((state, cost_so_far));

    states_mem.update_state(&state, min_cost, min_stack);
    return (min_cost, min_stack)
}

fn move_into_hall(cost_so_far: i32, rng: impl Iterator<Item = usize>, amp_row_num: usize, amp_col_num: usize, state: &StateParts, states_mem: &mut States) -> Vec<(i32, StackTrace)>{
